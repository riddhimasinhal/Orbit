...
const nicheOptions = ["Tech", "Education", "Fitness", "Fashion", "Beauty", "Gaming", "Travel", "Food", "Lifestyle"]
...
const BrowseCreators = () => {
    ...
    const fetchCreators = async () => {
        try {
            ...
            let url = "http://localhost:5001/api/creator/all";
            const params = [];
            if (searchText.trim()) {
    ...
    return (
        <div className="space-y-6">
            <div>
                ...
                <p className="text-sm text-zinc-500 mt-1">Find creators that match your brand's needs.</p>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
                ...
                {nicheOptions.map((niche) => (
                    <button
                        ...
                        onClick={() => setSelectedNiche(niche === selectedNiche ? "" : niche)}
                    >
                        {niche}
                    </button>
                ))}
            ...
            {loading ? (
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    {creators.map((creator) => (
                        <div key={creator._id} className="rounded-xl border border-white/10 bg-white/[0.03] p-5 hover:border-white/20 transition-all cursor-pointer" onClick={() => navigate("/brand/creator/" + creator._id)}>
                            ...
                            <div className="flex items-start gap-3 mb-3">
                                ...
                                <div className="flex-1 min-w-0">
                                    <h3 className="text-sm font-semibold text-white truncate">{creator.fullName || "Creator"}</h3>
                                    {creator.username && <p className="text-xs text-violet-400 truncate">@{creator.username}</p>}
                                    {creator.location && (
                            ...
                            {creator.niche && creator.niche.length > 0 && (
                                <div className="flex flex-wrap gap-1.5 mb-3">
                                    {creator.niche.map((n) => (
                                        <span key={n} className="rounded-full border border-violet-500/30 bg-violet-500/10 px-2 py-0.5 text-[10px] text-violet-300">
                                            {n}
                                        </span>
                                    ))}
                                </div>
                            ...
                            {/* bio */}
                            {creator.bio && (
                                <p className="text-xs text-zinc-400 mb-3 line-clamp-2">{creator.bio}</p>
                            )}
                            ...
                            <div className="flex items-center gap-4 pt-3 border-t border-white/5">
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-xs text-zinc-300">{formatNum(creator.instagramFollowers)}</span>
                                </div>
                                <div className="flex items-center gap-1.5">
                                    <Play className="size-3 text-zinc-500" />
                                    <span className="text-xs text-zinc-300">{formatNum(creator.youtubeSubscribers)}</span>
                                </div>
                                <div className="flex items-center gap-1.5">
                                    <Eye className="size-3 text-zinc-500" />
                                    <span className="text-xs text-zinc-300">{formatNum(creator.averageViews)}</span>
                                </div>
                            </div>
                        </div>
                    ))}

            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6">
                <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                        <h1 className="text-2xl font-semibold text-white">{profile?.companyName || "Brand"}</h1>
                        {profile?.industry && (
                        ...
                        <div className="flex flex-wrap gap-2 mt-3">
                            {profile?.preferredNiche?.length > 0 && profile.preferredNiche.map((n) => (
                ...
                {profile?.description && (
                    <p className="mt-4 text-sm text-zinc-400 border-t border-white/5 pt-4">{profile.description}</p>
                )}
            </div>
                ...
                <div className="rounded-xl border border-white/10 bg-white/[0.03] p-6">
                    ...
                    <div className="space-y-3">
                        <div className="flex justify-between">
                            ...
                            <span className="text-sm text-white max-w-[200px] text-right">{profile?.targetAudience || "Not set"}</span>
                        </div>
                        <div className="flex justify-between">
                            <span className="text-sm text-zinc-500">Marketing Goals</span>
                            <span className="text-sm text-white max-w-[200px] text-right">{profile?.marketingGoals || "Not set"}</span>
                        </div>
                        <div className="flex justify-between">
                        ...
                        <div className="flex justify-between">
                            ...
                            <span className="text-sm text-white">{profile?.preferredPlatform || "Not set"}</span>
                        </div>
                        <div className="flex justify-between">
                            <span className="text-sm text-zinc-500">Campaign Frequency</span>
                            <span className="text-sm text-white">{profile?.campaignFrequency || "Not set"}</span>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="space-y-3">
                        {profile?.contactPerson && (
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-sm text-white">{profile.contactPerson}</span>
                            </div>
                        )}
                        {profile?.contactEmail && (
                            <div className="flex items-center gap-3">
                                <Mail className="size-4 text-zinc-500" />
                                <span className="text-sm text-violet-400">{profile.contactEmail}</span>
                            </div>
                        )}
                        {profile?.website && (
                            <div className="flex items-center gap-3">
                                <Globe className="size-4 text-zinc-500" />
                                <span className="text-sm text-violet-400">{profile.website}</span>
                            </div>
                        )}
                        {profile?.instagramPage && (
                            <div className="flex items-center gap-3">
                                <Building className="size-4 text-zinc-500" />
                                <span className="text-sm text-violet-400">{profile.instagramPage}</span>
                            </div>
                        )}
                        {profile?.linkedInPage && (
                            <div className="flex items-center gap-3">
                                <Building className="size-4 text-zinc-500" />
                                <span className="text-sm text-violet-400">{profile.linkedInPage}</span>
                            </div>
                        )}
                    </div>
                </div>

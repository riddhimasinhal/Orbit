...
import { MapPin, AtSign, Play, Link2, Globe, Eye, ArrowLeft, Users, Send, Check, X } from "lucide-react"
...
const CreatorDetail = () => {
    ...
    useEffect(() => {
        const fetchCreator = async () => {
            try {
                ...
                const res = await axios.get("http://localhost:5001/api/creator/" + id, {
                ...
                if (res.data.creator?.userId) {
                    const connRes = await axios.get("http://localhost:5001/api/connections/check/" + res.data.creator.userId, {
                        headers: { Authorization: token },
                    })
    ...
    if (loading) return <p className="text-zinc-400">Loading creator profile...</p>
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6">
                <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                        <h1 className="text-2xl font-semibold text-white">{creator?.fullName || "Creator"}</h1>
                        {creator?.username && <p className="text-sm text-violet-400">@{creator.username}</p>}
                        {creator?.location && (
                ...
                {creator?.bio && (
                    <p className="mt-4 text-sm text-zinc-400 border-t border-white/5 pt-4">{creator.bio}</p>
                )}
                ...
                <div className="space-y-3">
                    {creator?.instagramUsername && (
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-sm text-violet-400">@{creator.instagramUsername}</span>
                        </div>
                    )}
                    {creator?.youtubeUrl && (
                        <div className="flex items-center gap-3">
                            <Play className="size-4 text-zinc-500" />
                            <a href={creator.youtubeUrl} target="_blank" className="text-sm text-violet-400 hover:underline">{creator.youtubeUrl}</a>
                        </div>
                    )}
                    {creator?.linkedInUrl && (
                        <div className="flex items-center gap-3">
                            <Link2 className="size-4 text-zinc-500" />
                            <a href={creator.linkedInUrl} target="_blank" className="text-sm text-violet-400 hover:underline">{creator.linkedInUrl}</a>
                        </div>
                    )}
                    {creator?.portfolioUrl && (
                        <div className="flex items-center gap-3">
                            <Globe className="size-4 text-zinc-500" />
                            <a href={creator.portfolioUrl} target="_blank" className="text-sm text-violet-400 hover:underline">{creator.portfolioUrl}</a>
                        </div>
                    )}
                    {!creator?.instagramUsername && !creator?.youtubeUrl && !creator?.linkedInUrl && !creator?.portfolioUrl && (

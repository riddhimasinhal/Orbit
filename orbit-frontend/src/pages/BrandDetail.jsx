...
import { MapPin, Building, Globe, ArrowLeft, AtSign, Link2, Mail, User, Send, Check, X } from "lucide-react"
...
const BrandDetail = () => {
    ...
    useEffect(() => {
        const fetchBrand = async () => {
            try {
                ...
                const res = await axios.get("http://localhost:5001/api/brand/" + id, {
                ...
                if (res.data.brand?.userId) {
                    const connRes = await axios.get("http://localhost:5001/api/connections/check/" + res.data.brand.userId, {
                        headers: { Authorization: token },
                    })
    ...
    const handleConnect = async () => {
        ...
        try {
            const token = localStorage.getItem("token")
            await axios.post("http://localhost:5001/api/connections/send", {
    ...
    if (error) {
        return (
            <div className="text-center py-16">
                ...
                <button onClick={() => navigate(-1)} className="mt-3 text-sm text-violet-400 hover:text-violet-300">Go back</button>
            </div>
        )
    }
        ...
        <div className="space-y-6">
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6">
                <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                        <h1 className="text-2xl font-semibold text-white">{brand?.companyName || "Brand"}</h1>
                        {brand?.industry && <p className="text-sm text-violet-400">{brand.industry}</p>}
                        {brand?.location && (
                ...
                {brand?.description && (
                    <p className="mt-4 text-sm text-zinc-400 border-t border-white/5 pt-4">{brand.description}</p>
                )}

                <div className="mt-4 pt-3">
                    {connectionStatus === null && (
                        <button onClick={handleConnect} disabled={sending} className="flex items-center gap-2 rounded-lg bg-violet-600 px-4 py-2 text-sm font-medium text-white hover:bg-violet-500 disabled:opacity-50">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4 text-center">
                    ...
                    <p className="text-lg font-semibold text-white">{brand?.companySize || "—"}</p>
                    <p className="text-xs text-zinc-500 mt-1">Company Size</p>
                </div>
                <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4 text-center">
                    <Globe className="size-5 text-violet-400 mx-auto mb-2" />
                    <p className="text-lg font-semibold text-white">{brand?.targetCountry || "—"}</p>
                    <p className="text-xs text-zinc-500 mt-1">Target Country</p>
                </div>
                <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4 text-center">
                    <p className="text-lg font-semibold text-white">{brand?.budgetRange || "—"}</p>
                    <p className="text-xs text-zinc-500 mt-1">Budget Range</p>
                </div>
                <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4 text-center">
                    <p className="text-lg font-semibold text-white">{brand?.campaignFrequency || "—"}</p>
            ...
            <div className="grid gap-4 lg:grid-cols-2">
                <div className="rounded-xl border border-white/10 bg-white/[0.03] p-6">
                    <h2 className="text-sm font-medium text-zinc-300 mb-4">Marketing Details</h2>
                    <div className="space-y-3">
                        <div className="flex justify-between">
                            <span className="text-sm text-zinc-500">Target Audience</span>
                            <span className="text-sm text-white">{brand?.targetAudience || "Not specified"}</span>
                        </div>
                        <div className="flex justify-between">
                            <span className="text-sm text-zinc-500">Marketing Goals</span>
                            <span className="text-sm text-white">{brand?.marketingGoals || "Not specified"}</span>
                        </div>
                        <div className="flex justify-between">
                            <span className="text-sm text-zinc-500">Preferred Platform</span>
                            <span className="text-sm text-white">{brand?.preferredPlatform || "Not specified"}</span>
                        </div>
                    </div>
                </div>

                <div className="rounded-xl border border-white/10 bg-white/[0.03] p-6">
                    <h2 className="text-sm font-medium text-zinc-300 mb-4">Contact & Social</h2>
                    <div className="space-y-3">
                        {brand?.contactPerson && (
                            <div className="flex items-center gap-3">
                                <User className="size-4 text-zinc-500" />
                                <span className="text-sm text-white">{brand.contactPerson}</span>
                            </div>
                        )}
                        {brand?.contactEmail && (
                            <div className="flex items-center gap-3">
                                <Mail className="size-4 text-zinc-500" />
                                <span className="text-sm text-violet-400">{brand.contactEmail}</span>
                            </div>
                        )}
                        {brand?.website && (
                            <div className="flex items-center gap-3">
                                <Globe className="size-4 text-zinc-500" />
                                <a href={brand.website} target="_blank" className="text-sm text-violet-400 hover:underline">{brand.website}</a>
                            </div>
                        )}
                        {brand?.instagramPage && (
                            <div className="flex items-center gap-3">
                                <AtSign className="size-4 text-zinc-500" />
                                <span className="text-sm text-violet-400">{brand.instagramPage}</span>
                            </div>
                        )}
                        {brand?.linkedInPage && (
                            <div className="flex items-center gap-3">
                                <Link2 className="size-4 text-zinc-500" />
                                <a href={brand.linkedInPage} target="_blank" className="text-sm text-violet-400 hover:underline">{brand.linkedInPage}</a>
                            </div>
                        )}
                    </div>
                </div>

...
const BrowseBrands = () => {
    ...
    const fetchBrands = async () => {
        try {
            ...
            let url = "http://localhost:5001/api/brand/all";
            const params = [];
            if (searchText.trim()) {
    ...
    return (
        <div className="space-y-6">
            <div>
                ...
                <p className="text-sm text-zinc-500 mt-1">Discover brands looking for creators like you.</p>
            </div>
            ...
            {loading ? (
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    {brands.map((brand) => (
                        <div key={brand._id} className="rounded-xl border border-white/10 bg-white/[0.03] p-5 hover:border-white/20 transition-all cursor-pointer" onClick={() => navigate("/creator/brand/" + brand._id)}>
                            <div className="flex items-start gap-3 mb-3">
                                ...
                                <div className="flex-1 min-w-0">
                                    <h3 className="text-sm font-semibold text-white truncate">{brand.companyName || "Brand"}</h3>
                                    {brand.industry && <p className="text-xs text-violet-400 truncate">{brand.industry}</p>}
                                    {brand.location && (
                            ...
                            {/* description */}
                            {brand.description && (
                                <p className="text-xs text-zinc-400 mb-3 line-clamp-2">{brand.description}</p>
                            )}
                            ...
                            <div className="flex items-center gap-4 pt-3 border-t border-white/5">
                                {brand.companySize && (
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        <span className="text-xs text-zinc-300">{brand.companySize}</span>
                                    </div>
                                )}
                                {brand.website && (
                                    <div className="flex items-center gap-1.5">
                                        <Globe className="size-3 text-zinc-500" />
                                        <span className="text-xs text-violet-400 truncate">{brand.website}</span>
                                    </div>
                                )}
                                {brand.budgetRange && (
                                    <span className="text-xs text-zinc-300">{brand.budgetRange}</span>
                                )}
                            </div>
                        </div>
                    ))}
